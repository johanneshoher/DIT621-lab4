...
CREATE OR REPLACE FUNCTION insert_student_reg(param1 TEXT, param2 TEXT) RETURNS VOID AS $$
BEGIN
  INSERT INTO Registered VALUES (param1, param2);
END;
$$ LANGUAGE plpgsql;
...
DECLARE 
    ...
    sts TEXT;
    cap INT;
    waiting_student TEXT;
    numStudents INT;
BEGIN
    old_course := OLD.course;

    -- Is not actually neccesary, because the trigger never finds anyone to delete, it will not call the function.
    ...
    IF EXISTS (SELECT student, course, status FROM Registrations r WHERE r.student = OLD.student AND r.course = old_course) THEN
        ...
        IF (sts = 'registered') THEN
            ...
            IF EXISTS (SELECT 1 FROM LimitedCourses l WHERE l.code = old_course) THEN
                SELECT capacity INTO cap FROM LimitedCourses l WHERE l.code = old_course;
                
                SELECT COUNT(student) INTO numStudents FROM Registrations r WHERE r.course = old_course AND r.status = 'registered';

                    IF EXISTS (SELECT 1 FROM CourseQueuePositions c WHERE c.course = old_course AND c.place = 1) 
                        AND (numStudents < cap)
                        THEN
                        -- RAISE EXCEPTION 'HERE, FAULT';
                        SELECT student INTO waiting_student FROM CourseQueuePositions c WHERE c.course = old_course AND c.place = 1;

                        DELETE FROM WaitingList c WHERE c.course = old_course AND c.position = 1;
                        
                        PERFORM decrement_place(old_course, 1);
                        
                        PERFORM insert_student_reg(waiting_student, old_course);
